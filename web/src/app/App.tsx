...
export function App() {
  ...
  return <div className="app-shell">
    <aside className="desktop-rail"><div className="brand"><span className="brand-mark">d</span><span>DSH <b>Mobile</b></span></div><p className="rail-label">YOUR WORKSPACE</p>{["inbox", "tasks", "sessions"].map(name => <button key={name} className={tab === name && !selected ? "selected" : ""} onClick={() => { back(); setTab(name); }}><Icon name={name}/>{name[0].toUpperCase() + name.slice(1)}{name === "inbox" && needs.length > 0 && <span className="count">{needs.length}</span>}</button>)}<div className="rail-bottom"><span className={"dot " + (online ? "live" : "")}/>{state.host?.name ?? "Connecting to host"}</div></aside>
    <div className="main-column">
      <header className="topbar"><div className="host-identity"><span className="brand-mark small">d</span><div><strong>DSH Mobile</strong><small>{state.host?.name ?? "Your agent, within reach"}</small></div></div><button className="icon-button" aria-label="Settings" onClick={() => { back(); setTab("settings"); }}><Icon name="settings"/></button></header>
