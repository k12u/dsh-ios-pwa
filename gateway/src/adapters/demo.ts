...
import type { ApprovalResponse, Capability, MobileEvent, PresetsDTO, QuestionResponse, SelectModelInput, SendPromptInput, SetPresetInput, Snapshot } from "@dsh-mobile/protocol";
...
export class DemoAdapter implements HarnessAdapter {
  ...
  capabilities(): Capability[] { return ["sessions", "workspaces", "streaming", "tasks", "approvals", "questions", "images", "cancel", "steer", "models", "presets"]; }
  ...
  async presets(): Promise<PresetsDTO> {
    return { presets: this.roster.map(p => ({ ...p, default: p.id === this.preset })) };
  }
  async selectPreset(input: SetPresetInput) {
    if (!this.roster.some(p => p.id === input.presetId)) throw new GatewayError(400, "unknown-preset", "That agent mode is not available.");
    this.preset = input.presetId;
  }
