...
Harness 本体の fork・同梱・native wrapper はありません。GitHub 上の fork や公開リポジトリの作成は行っていません。dsh1 の tailnet 内に受入試験用 Gateway を配置しています。
...
Harness smoke は一時的な `DSH_HOME` を使い、公式 runtime + plugin 起動・static serve・pairing・host/workspace/session snapshot・WebSocket の hello / snapshot 初期同期を確認します。モデルを使う prompt/HITL/image の全経路は別途実接続で検証が必要です。
